    ...
    >
        <div v-if="multiple" ref="tags" class="el-select__tags" :style="{ 'max-width': inputWidth - 32 + 'px', width: '100%' }">
            ...
            <transition-group v-if="!collapseTags" @after-leave="resetInputHeight">
                <el-tag v-for="item in selected" :key="getValueKey(item)" :closable="!selectDisabled" :size="collapseTagSize" :hit="item.hitState" type="info" disable-transitions @close="deleteTag($event, item)">
        ...
        <transition name="el-zoom-in-top" @before-enter="handleMenuEnter" @after-leave="doDestroy">
            <el-select-menu v-show="visible && emptyText !== false" ref="popper" :append-to-body="popperAppendToBody">
                ...
                <template v-if="emptyText && (!allowCreate || loading || (allowCreate && filteredOptions.length === 0))">
...
export default {
    ...
    props: {
        ...
        virtual: { type: Object, default: () => ({ dataKey: 'value', minItemSize: 34 }) }
    },
    data() {
        return {
            optionsList: [],
    ...
    computed: {
        filteredOptions() {
            const { filterQuery: query, onQueryOption } = this;
            return this.optionsList.reduce((pre, option) => {
                if(Array.isArray(option.options)) {
                    ...
                    if(options.length > 0) {
                        pre.push({ ...option, type: "group", value: option.value || option.label }, ...options);
                    }
                } else {
                    if(query) {
        ...
        showNewOption() {
            let hasExistingOption = this.filteredOptions.filter(option => !option.created)
